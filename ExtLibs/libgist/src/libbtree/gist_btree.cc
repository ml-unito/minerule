    ...
    int& numMatches)
{
	...
	// Modified by Marco Botta: find last entry with key = q->val1
	//end = page.nrecs() - 1;
	end = _binSearch(page, q->val1, NULL, true, true);
	break;
	...
	if (start == -1) start = 0;
	// Modified by Marco Botta: find last entry with key = q->val1
        end = _binSearch(page, q->val2, NULL, true, true);
	break;
...
bt_ext_t::_binSearch(
    ...
    const void* data,
    bool keyOnly,
	 // Modified by Marco Botta: added parameter firstOrLast. It says if _binSearch should position on the first or last element equal to key
	 bool firstOrLast) // true: only compare keys
{
    int hi = page.nrecs() - 1;
    if (hi == -1) {
	...
	if (res < 0) {
		...
		// Modified by Marco Botta: set lo or hi according to firstOrLast
	  if (firstOrLast) {
			lo = mid;
			if (hi == lo || hi-1 == lo) return hi;
	  } else {		
...
bt_ext_t MR_bt_int_ext(gist_ext_t::MR_bt_int_ext_id, "MR_bt_int_ext",
    gist_support::printIntBtPred, printString,
    gist_support::parseInt, gist_support::parseString,
    gist_support::parseIntQuery, int_cmp, str_cmp,
    int_size, str_size, int_negInfty, str_negInfty);

bt_ext_t MR_bt_str_ext(gist_ext_t::MR_bt_str_ext_id, "MR_bt_str_ext",
    gist_support::printStringBtPred, printString,
