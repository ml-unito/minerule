...
namespace minerule {
  ...
  void
  GAQueryCombinator::buildAssociatedPredicate(const GAGenome& g, 
					Predicate& p,
    ...
    const GA1DBinaryStringGenome& genome = 
      dynamic_cast<const GA1DBinaryStringGenome&>(g);
  ...
  float GAQueryCombinator::operator()(GAGenome& g) {
    ...
    float ham_distance=
      EncodedNF::computeHammingDistance(e1.encode(p1),e2.encode(p2));
    ...
    return ( (1<<maxDistinctPredicates)* HAM_LOSS 
	     + mr_candidates.size()*maxDisjuncts 
	     - ham_distance*HAM_LOSS 
	     - numAssertedBits);
  ...
  void GAQueryCombinator::buildResult(const GAGeneticAlgorithm& ga) {
    ...
    const GA1DBinaryStringGenome& solution =
      dynamic_cast<const GA1DBinaryStringGenome&>(ga.population().best());
