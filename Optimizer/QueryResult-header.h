...
namespace minerule {
  ...
  public:
    ...
    public:
      Iterator() : 
	state(NULL), rs_rules(NULL), ps_elems(NULL) {}
	...
	if( ps_elems!=NULL ) 
       ...
       */
      void init( const std::string& rulesTable,
		 double support,
		 double confidence ) throw( MineruleException, odbc::SQLException );

      /**
       * position on the following rule. The first time it is called
      ...
      void getRule( QueryResult::Rule& r ) 
	throw( MineruleException, odbc::SQLException, std::exception );
    };
    ...
    template <class Sorter>
      ...
      void load(const std::string& qryName, 
		double sup=-1,
		double con=-1) throw(MineruleException);
