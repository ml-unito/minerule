...
public:
    ...
    int readSeqVal(int id_seq){
        ...
        odbc::ResultSet* rs= execQr(seqIDQuery);
        rs->next();
        int val=rs->getInt(1);
        return val;
    }

    odbc::ResultSet* execQr(std::string sqlQuery){
    ...
    int getTrackidMaxValue(){
        ...
        odbc::ResultSet* rs=options.getODBCConnection()->prepareStatement(ODBCXX_STRING_CONST(query.c_str()))->executeQuery();
        rs->next();
        int val=rs->getInt(1);
        return val;
    }
