...
private:
  ...
  std::vector<std::string> AttrList; // lista di tutti gli attr coinvolti nella
...
public:
  STSMinerWithBitVector(const OptimizedMinerule &mr,
                        const AlgorithmsOptions &opts)
      : MiningAlgorithm(mr), pm(mr.getParsedMinerule()),
        pdu(pm, this->sourceTableRequirements()) {
    options = opts;
    ...
    min_space_gap = 0;
    /*************************************/
    // il parser va modificato perchè legga le clausole begin/end per la
    // seguente inzializzazione
    ...
    if (!check(begin_e_operator)) {
      MRDebug() << "Wrong operator definition in begin clause!" << std::endl
                << "Allowed operator: = , > , < , >= , <= , !=" << std::endl;
    ...
    if (!check(end_t_operator)) {
      MRDebug() << "Wrong operator definition in end clause!" << std::endl
                << "Allowed operator: = , > , < , >= , <= , !=";
      std::cout << std::endl << "Using default operator : =" << std::endl;
      end_t_operator = "=";
    }
    ...
    for (int i = 0; i < pm.oa.size(); i++) {
      for (int j = 0; j < AttrList.size(); j++)
        // se l'attributo era già tra quelli del body lo segnamo come found e
        // non lo aggiungiamo
        if ((AttrList.at(j)).compare(pm.oa.at(i)) == 0)
          found = true;
      if (!found)
        AttrList.push_back(pm.oa.at(i));
      found = false;
    }
    found = false;
    for (int i = 0; i < pm.ba.size(); i++) {
      for (int j = 0; j < AttrList.size(); j++)
        // se l'attributo è già stato inserito lo segnamo come found e non lo
        // aggiungiamo
        if ((AttrList.at(j)).compare(pm.ba.at(i)) == 0)
          found = true;
      if (!found)
        AttrList.push_back(pm.ba.at(i));
      found = false;
    }
  ...
  void createSeqQ(int idseq) {
    ...
    createSeqIDQuery +=
        " START 1 CACHE 1;"; // " ALTER TABLE id" + id_s + "_sq OWNER TO postgres;";
    execStatement(createSeqIDQuery);
  }
  ...
  void execStatement(std::string sqlQuery) {
  ...
  void dropAllTables() {
    ...
    for (int j = 0; j < 10; ++j) {
      dropQuery = "";
      for (int i = 0; i < 60; ++i) { // 100?! è da vedere come farlo questo...
        dropQuery += "DROP TABLE IF EXISTS Seq" + Converter(j*60+i).toString() + ";";
        dropQuery +=
            "DROP SEQUENCE IF EXISTS id" + Converter(j*60+i).toString() + "_sq;";
      }
  ...
  bool existsTable(int k) {
    ...
    std::string q =
        "IF EXISTS Seq" + strK + "SELECT * FROM Seq" + strK + "LIMIT 1";
  ...
  void showResults(int k) {
    ...
    while (rs->next()) {
      ...
      if (begin_end_vect.at(0).at(j)->count(true) == 2) {
        std::cout << "<" + subarea_id + "," + Converter(d_time).toString() +
                         "> " << std::endl;
      }
      ++j;
    }
