...
/* La seguente classe crea un oggetto STSMiner il quale data la MINE SEQUENCE in
input elabora
in automatico delle query SQL per estrarre le sequenze frequenti dal db
delegandogli così il grosso del lavoro*/

/*
L'algoritmo accetta un attributo per l'ordinamento ed una attributo per gli
elementi e restituisce
sequenze di coppie <el_attr,ord_attr> che rispettano il vincolo del supporto e
del gap inteso come gap
tra i valori dell'attributo di ordinamento tra due elementi(coppie) successivi
della sequenza.
...
/*da correggere: la tipologia fissa degli elementi in Source non va molto
bene... lines 25-27
...
bool STSMinerWithBitVector::extract1Sequences() {
  ...
  std::string dropQuery =
      " DROP TABLE IF EXISTS Seq1;DROP TABLE IF EXISTS Seq2;";
  dropQuery += "DROP SEQUENCE IF EXISTS id2_sq; DROP TABLE IF EXISTS Source; "
               "DROP SEQUENCE IF EXISTS idsrc_sq;DROP SEQUENCE IF EXISTS "
               "id1_sq;";
  STSMinerWithBitVector::execStatement(dropQuery);

  createSeqQ(1);
  std::string createTableQuery = "CREATE TABLE Source ( " + AttrList.at(0) +
                                 " integer, " + AttrList.at(1) + " numeric, " +
  ...
  sqlQuery = sqlQuery + " FROM " + pm.tab_source + " " + " ORDER BY " +
             ordAttrList + " , " + groupAttrList + ";";
  execStatement(sqlQuery);
...
bool STSMinerWithBitVector::pruning1Seq() {
  ...
  /*LE SEGUENTI RIGHE GENERANO UNA TAB A PARTE PER LE SEQUENZE LUNGHE 1 CON
   * SUPPORTO SUFFICENTE*/
  std::string sqlQuery = "DROP TABLE IF EXISTS Seq1; CREATE TABLE Seq1";
  sqlQuery += "(" + ordAttrList + " numeric, " + AttrList.at(2) +
              " character varying(10),";
  sqlQuery +=
      "ID integer NOT NULL DEFAULT nextval('id1_sq'::regclass) PRIMARY KEY);";
  execStatement(sqlQuery);

  sqlQuery = "INSERT INTO Seq1 SELECT " + ordAttrList + "," + AttrList.at(2);
  sqlQuery += " FROM Source GROUP BY " + ordAttrList + " , " + AttrList.at(2);
  sqlQuery += " HAVING COUNT( DISTINCT " + groupAttrList + ")>=" +
              Converter(pm.sup * seqid_count).toString();
  execStatement(sqlQuery);
...
void STSMinerWithBitVector::createBitVectors() {
  ...
  // per ogni singoletto creiamo un vettore di bv_length bit in cui ogni entry
  // rappresenta un trackid
  ...
  while (rs->next()) {
    ...
    int id = rs->getInt(3);
    q = "SELECT " + groupAttrList + " FROM Source WHERE " + ordAttrList + "=" +
        Converter(d_time).toString() + " AND " + AttrList.at(2) + "='" +
        Converter(subarea_id).toString() + "';";
    mrdb::ResultSet *ris = connection.getMRDBConnection()
                               ->prepareStatement(q.c_str())
                               ->executeQuery();
    while (ris->next()) {
    ...
    if (begin_e == "" && begin_t == "") {
      ...
      *v = v->set(0, true);
      ...
      if (begin_e != "" &&
          checkConstraint(subarea_id, begin_e, begin_e_operator)) {
        BitString *v = begin_end_vect.at(0).at(id - 1);
        *v = v->set(0, true);
      }
      if (begin_t != "" && checkConstraint(Converter(d_time).toString(),
                                           begin_t, begin_t_operator)) {
        BitString *v = begin_end_vect.at(0).at(id - 1);
        *v = v->set(0, true);
      }
    }
    if (end_e == "" && end_t == "") {
      ...
      *v = v->set(1, true);
      ...
      if (end_t != "" && checkConstraint(Converter(d_time).toString(), end_t,
                                         end_t_operator)) {
        BitString *v = begin_end_vect.at(0).at(id - 1);
        *v = v->set(1, true);
      }
...
bool STSMinerWithBitVector::extract2Sequences() {
  ...
  std::string createTableQuery =
      "CREATE TABLE Seq2 (ID_1 integer,ID_2 integer)";
  execStatement(createTableQuery);
  ...
  std::string sqlQuery =
      "INSERT INTO Seq2 SELECT s_1.ID AS ID_1, s_2.ID AS ID_2 ";
  sqlQuery += "FROM Seq1 s_1, Seq1 s_2, Source s1, Source s2 ";
  sqlQuery += "WHERE s_1." + ordAttrList + "=s1." + ordAttrList + " AND s_2." +
              ordAttrList + "=s2." + ordAttrList + " ";
  sqlQuery += "AND s_1." + AttrList.at(2) + "=s1." + AttrList.at(2) +
              " AND s_2." + AttrList.at(2) + "=s2." + AttrList.at(2) + " ";
  sqlQuery += "AND s1." + groupAttrList + "=s2." + groupAttrList + " ";
  sqlQuery +=
      "AND s1." + ordAttrList + "<=s2." + ordAttrList + "-" + strmin_g + " ";
  sqlQuery +=
      "AND s1." + ordAttrList + ">=s2." + ordAttrList + "-" + strmax_g + " ";
  ...
  sqlQuery += Converter(max_space_gap).toString() + " AND " +
              Converter(min_space_gap).toString();
  ...
  /*sqlQuery += "AND NOT EXISTS(SELECT ";
   ...
   sqlQuery += "WHERE s_3.d_time=s3.d_time AND s_3.subarea_id=s3.subarea_id AND
   s3.trackid=s1.trackID ";
...
bool STSMinerWithBitVector::extractKSequences(int k) {
  ...
  std::string dropQuery = " DROP TABLE IF EXISTS Seq" + strK +
                          "; DROP SEQUENCE IF EXISTS id" + strK + "_sq;";
  execStatement(dropQuery);

  std::string createTableQuery = "DROP TABLE IF EXISTS Seq" + strK +
                                 "; CREATE TABLE Seq" + strK +
                                 " (ID_1 integer,ID_2 integer)";
  execStatement(createTableQuery);

  std::string sqlQuery =
      "INSERT INTO Seq" + strK + " SELECT s1.ID AS ID_1, s2.ID AS ID_2 ";
...
bool STSMinerWithBitVector::pruningSeqK(int k) {
  ...
  while (rs->next()) {
    ...
    if (tmp0->count(true) >= (pm.sup * seqid_count)) {
      new_bvptr.push_back(tmp0);
      new_begin_end_vect.push_back(ris);
    } else
      execStatement("DELETE FROM Seq" + strK + " WHERE ID_1=" +
             Converter(id1).toString() + " AND ID_2=" +
             Converter(id2).toString());
  }
  ...
  if (!emptyTable("Seq" + strK)) {
    createSeqQ(k);
    query = "ALTER TABLE Seq" + strK +
            " ADD ID integer NOT NULL DEFAULT nextval('id" + strK +
            "_sq'::regclass) PRIMARY KEY ";
    execStatement(query); // L'AGGIUNTA DELL'ID SAREBBE MEGLIO FARLA DOPO IL PRUNING
                   // COSÌ HA VALORI CONTINUI
    return false;
  } else
    return true;
}
...
void STSMinerWithBitVector::sourceFilter() {
  // se sono state specificate clausole begin e/o end filtriamo source
  // eliminando tutte le track
  // che non contengono almeno un'occorrenza di questi elementi
  if (begin_e != "" || begin_t != "" || end_t != "" || end_e != "") {
    std::string sq = "DELETE FROM Source WHERE trackid NOT IN( SELECT "
                     "s1.trackid FROM Source s1, Source s2 ";
    sq += "WHERE s1." + groupAttrList + "=s2." + groupAttrList + " AND ";
    bool addAND = false; // se viene impostato a true quando aggiungiamo il
                         // begin_e dobbiamo precederlo con un AND
    if (begin_t != "") {
  ...
  do {
    ...
    if (k == 1) {
      ...
      bv_length =
          ...
          1; // il range di valori dei trackid è la lunghezza dei bitvector
      ...
      if (!EmptyResult) {
        ...
        if (EmptyResult)
          MRLog() << "Table Seq" + strK + " now is empty..." << std::endl;
      }
    }
