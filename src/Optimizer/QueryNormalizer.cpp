  ...
  */
  ...
  void QueryNormalizer::compactPredicates( list_AND_node*& l ) const {
	...
	continue;
      ...
      list_AND_node** Yptr;
      list_AND_node* oldCell=(*Xptr)->next;
      ...
      for( Yptr=&(*Xptr)->next; *Yptr!=NULL; 
	   *Yptr==oldCell?(Yptr=&(*Yptr)->next):0 ) {
	oldCell=*Yptr;
	...
	default:
      ...
      // need this check since we may have decided to delete a node
      // just before ending the cycle.
      //      if(nodeToRemove!=NULL) {
      //	removeNode(nodeToRemove);
      //	nodeToRemove=NULL;
      //      }
