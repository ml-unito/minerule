...
class MIndex {
   ...
   std::string concatenate(std::vector<int>& v);
	...
	MIndex(const char *tableName, const char *itemColName, std::vector<int>& gr, std::vector<int>& itCols) :
	  	name(tableName), itemCol(itemColName), groupCol(gr), itemCols(itCols) {
	...
	static bool createIndex(const char *tableName, const char *itemColName, std::vector<int>& gr, std::vector<int>& itCols);
